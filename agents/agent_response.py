...
from integrations.llm_wrapper import HumanMessage, LLMAPIWrapper, SystemMessage, AIMessage
from prompt_management.prompts import REACT_STEP_POST, REACT_STEP_PROMPT, REACT_SYSTEM_PROMPT, REACT_PLAN_PROMPT, STATIC_PRE_PROMPT, STATIC_PRE_PROMPT_PRIME, REACT_STEP_PROMPT_PRIME, REACT_STEP_POST_PRIME
...
class AgentResponse:
    ...
    def _build_react_prompt(self, input_text: str, conversation_accumulator: str, thought_number: int, action_number: int) -> str:
        ...
        return f"Question: {input_text}\n{conversation_accumulator}\nThought {thought_number}: {thought_prompt}\nAction {action_number}: {action_prompt}"

    def _generate_chat_response(self, system_prompt: str, react_prompt: str) -> AIMessage:
        """
        ...
        """
        return self.llm_wrapper.chat_completion([SystemMessage(system_prompt), HumanMessage(react_prompt)])

    def _process_response(self, response: AIMessage, conversation_accumulator: str, thought_number: int, action_number: int, input_text: str) -> Tuple[str, int, int]:
        """
        ...
        thought_number += 1
        action_number += 1
        self.agent.update_status("Processing response")
        string_response = response.pretty_repr()
        if "```python" in string_response:
            self.agent.update_status("Executing Python code")
            self.agent.number_of_code_executions += 1
            if code_execution_ability := self.agent.get_ability("CodeExecutionAbility"):
                exec_response = code_execution_ability.execute(self.agent, text_with_code=string_response)
                conversation_accumulator += f"\nObservation: Executed Python code\nOutput: {exec_response}"

        if "Use Agent[" in string_response:
            agent_name, input_text = self._parse_agent_info(string_response)
    ...
    def _conclude_output(self, conversation: str) -> str:
        ...
        return self.llm_wrapper.chat_completion(messages=[SystemMessage(REACT_SYSTEM_PROMPT), HumanMessage(conversation)]).pretty_repr()
